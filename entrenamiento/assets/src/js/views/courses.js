...
async function renderAdminCoursesView(container, user) {
    try {
            ...
            <div class="container">
                ...
                ${courses.length === 0 ? `
                    <div class="alert alert-info" style="margin-top: 2rem;">
                        No hay cursos creados todavía. Haz clic en "Crear Nuevo Curso" para comenzar.
                    </div>
                ` : ''}
            </div>
        `;
        ...
        container.innerHTML = '<div class="container"><div class="alert alert-error">Error al cargar los cursos</div></div>';
    }
}
...
async function enrollInCourse(courseId) {
    ...
    try {
        ...
        if (course.enrolledStudents && course.enrolledStudents.includes(user.id)) {
            showMessage('messageContainer', 'Ya estás inscrito en este curso', 'error');
            return;
        }
        ...
        console.error('Error:', error);
        showMessage('messageContainer', 'Error al inscribirse en el curso', 'error');
    }
}
